...
describe('Games', () => {
    ...
    const eventEmitter = { publish: function (...args) { this.receivedArgs = args } }
    ...
    it('publishes an event when player joins the game, sets score and avatar', async () => {
        const adapter = new Games(dummyTimer, dummyQuizRepo, eventEmitter)
        const gameId = await adapter.host()
        adapter.join(gameId, 'alice', null)
...
fit('publishes an event when game is started with first question + time to guess', (done) => {
