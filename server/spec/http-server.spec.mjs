...
describe('Server', () => {
    ...
    describe('endpoint protection', () => {
        ...
        beforeAll(() => {
            ...
            server = httpServer(new GoogleAuth(dummyConfig), { dir: '' }, url, noExpiryTimer, null, null)
            server.listen(port)
            client = request(url)
        })
    ...
    describe('game API', () => {
        ...
        beforeAll(async () => {
            ...
            port = 3002
            url = `http://localhost:${port}`
            server = httpServer(dummyAuth, quizService, url, noExpiryTimer, null, null)
            server.listen(port)
            client = request(url)
        })
    ...
    describe('history API', () => {
        ...
        beforeAll(async () => {
            ...
            port = 3003
            url = `http://localhost:${port}`
            server = httpServer(dummyAuth, quizService, url, noExpiryTimer, historyService, null)
            server.listen(port)
            client = request(url)
        })
